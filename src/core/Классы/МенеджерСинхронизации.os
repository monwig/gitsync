...
//   ОтключитьЗаменуАвтора - Булево - признак отключения замены автора коммита при изменении версии конфигурации
//   СлужебныйПользовательДляЗамены - Строка - служебный пользователь git для замены автора
...
Процедура Синхронизировать(Знач КаталогРабочейКопии, 
							Знач ПутьКХранилищу,
							Знач ОтключитьЗаменуАвтора,
							Знач СлужебныйПользовательДляЗамены,
	...
	Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			Попытка
				...
				ПредыдущаяВерсияКонфигурации = ПолучитьВерсиюКонфигурации(КаталогРабочейКопии);
				Лог.Отладка("Предыдущая версия конфигурации: " + ПредыдущаяВерсияКонфигурации);

				ВыгрузитьВерсиюХранилищаКонфигурацииВИсходники(КаталогРабочейКопии, ПутьКХранилищу, СледующаяВерсия);
				
				НоваяВерсияКонфигурации = ПолучитьВерсиюКонфигурации(КаталогРабочейКопии);
				Лог.Отладка("Следующая версия конфигурации: " + НоваяВерсияКонфигурации);

				Если НЕ ОтключитьЗаменуАвтора 
					И (НоваяВерсияКонфигурации <> "не определена" И ПредыдущаяВерсияКонфигурации <> "не определена")
					И (НоваяВерсияКонфигурации <> ПредыдущаяВерсияКонфигурации) Тогда
					
					Лог.Информация("В версии " + СледующаяВерсия + " выполнено типовое обновление. Произведена замена автора.");
					
					СтрокаВерсии.ПредставлениеАвтора = СлужебныйПользовательДляЗамены;
				КонецЕсли;
...
Функция ПолучитьВерсиюКонфигурации(Знач КаталогИсходников)

	Попытка
		ПроцессорXML = Новый СериализацияДанныхXML();
		СодержимоеXML = ПроцессорXML.ПрочитатьИзФайла(ОбъединитьПути(КаталогИсходников, "Configuration.xml"));

		Возврат СодержимоеXML["MetaDataObject"]["_Элементы"]["Configuration"]["_Элементы"]["Properties"]["Version"];
	Исключение
